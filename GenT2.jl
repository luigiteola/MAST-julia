...
for gen_name in GenT2
    ...
    # Get the maximum real power of the generator
    ...
    if gen_type in Solar_Tech
        ...
        for t in Time
            trace_value = Solar_trace_DR[(bus_name, t)] * max_real_power
    ...
    elseif gen_type in Wind_Tech
        ...
        for t in Time
            trace_value = Wind_trace_DR[(bus_name, t)] * max_real_power
            Resource_trace_T2[(gen_name, t)] = trace_value
        end
    ...
    println("Processed generator: ", gen_name, " of type: ", gen_type, " at bus: ", bus_name, " with capacity: ", max_real_power)

    # Print trace values for this generator
    gen_vector = [Resource_trace_T2[(gen_name, t)] for t in Time]
    formatted_values = join(round.(gen_vector, digits=3), " ")  # Space-separated, rounded to 3 decimals
    println("Trace values for generator ", gen_name, ": ", formatted_values)
